...
(th~deftheorem fact4
	     (in prop-res)
	     (conclusion (forall (lam (klm (o (o form))) (forall (lam (kl (o form))
			     (forall (lam (l form)
			       (implies
				(and
				 (and (klm kl) (kl l))
	     ...
	     (help "A lemma reasonning about the concatenability of derivations."))
...
(th~defaxiom fact5
	     (in prop-res)
	     (formula (forall (lam (y (o (o form))) (forall (lam (z (o form))
		      (forall (lam (x glist)
			 (implies (and (= (minus (glist-length x) one)
					  zero)
				       (derivation-of y crule-set x z))
				  (= x
				     (cons (der-item-constr res-start z NULL NULL)
					   NULL)))))))))))
