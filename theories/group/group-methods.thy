...
;                 (th-restricted-definition (:symbol :base) image-of-domain image-def)))
;
;                (outline-computations
;                 (image-expand (beta-normalize (appl-create image-def (mlist fun dom ranelem)))))
;
;               (decl-content 
;                (l05 () (IMAGE-OF-DOMAIN FUN dom ranelem))
...
(meth~defmethod element-of-domain-m-f element-of-domain-m
		...
		(l05 () (IMAGE-OF-DOMAIN FUN dom ranelem)              )
		(p10 () (exists-sort (lam (x aaa) (= ranelem (fun x))) dom ) ("defne" (image-of-domain image-def)(l05)))
		(l07 (l00) conc                                         ("open" () ())) 
...
;                (l10 () (IMAGE-OF-DOMAIN FUN dom ranelem)) ("solved" () (l05 l07))) 
;                )
...
;                (unify phi const))
               ...
               (decl-content
                ...
                (l05 () (IMAGE-OF-DOMAIN FUN dom const) ("open" ()()))
...
(meth~defmethod group-closed-m-b group-closed-m
		...
		(mand ;(mbind elem1 (newmetavar (:symbol celem) aaa))
		 ...
		 (th-restricted-definition (:symbol :base) closed-under-2 closed-def)))
		...
		(l01 () (grp elem1) ("open" () ()))
		(l02 () (grp elem2) ("open" () ()))
	        (l10 () (grp (op elem1 elem2)) ("foralle-sort*" (tl) (p40 l01 l02)))

		(p10 () group-expand                                  ("defne" (group group-def)(l00)))
		(p20 ()	 (and (closed-under-2 grp op)
			      (associative grp op))		      ("andel" ()(p10)))
		(p30 ()	 (closed-under-2 grp op)           	      ("andel" ()(p20)))
		(p40 ()	closed-expand		                      ("defne" (closed-under-2 closed-def)(p30)))
                )
...
(meth~defmethod element-of-kernel-m-f element-of-kernel-m
		 ...
		 (delem aaa term)        (relem bbb term)))
	       ...
	       (outline-computations)
               ...
               (decl-content
		(l00 () (KERNEL-FOR-ELEM FUN dom relem delem))
...
(meth~defmethod subset-kernel-domain-m-b subset-kernel-domain-m
		...
		(mfalse));                (unify phi const))
               ...
               (decl-content
                (l00 () (HOMOMORPHISM  dom OP ran OP1 fun))
                (l05 () (KERNEL-FOR-ELEM FUN dom (GROUP-UNIT ran OP1) const) ("open" ()()))
