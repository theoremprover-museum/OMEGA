...
(defun equation? (thetask)
  (let* ((goal-tasks (remove-if-not #'agenda~goal-or-goal-schema-task-p cri*current-tasks))
	 (equation-tasks (remove-if-not #'(lambda (task)
						 (frame~equality? (agenda~task-node task)))
					   goal-tasks)))
...
(defun get-defis (defis)
  (labels ((more-abstract (def1 def2)
			  (or (< (length def1)(length def2))
			      (< (count-constants def1)
				 (count-constants def2))))
	    (count-constants (termlist)
			     (apply #' + (mapcar #'(lambda (def)
						     (length (remove-if-not #'term~constant-p  (data~all-substructs def))))
						 termlist))))
    (unless (frame~logic? cri*current-task-node)
      (let ((definitions (sort (frame~get-definitions
					;(pds~label2node 'image-closed))
				cri*current-task-node)
			       #'more-abstract)))
	(mapcar #'(lambda (def)
;		    (omega~trace "~%defi: ~A~%" def)
		    (mapp~create (list defis)(list def)))
		definitions)))))
