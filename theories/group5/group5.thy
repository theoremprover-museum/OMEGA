		       ...
		       (and (not-empty G)
			    (and (closed-under G op)
				 (and (associative G op)
				      (and (exists-sort (lam (e aa) (left-unit G op e)) G)
					   (left-inverse-exist G op (struct-left-unit G op)))))))))
...
(th~defdef group-unit 
	   ...
	   (type-variables bb)
		  ...
		  (lam (op (bb bb bb))
		       (struct-unit G op))))
	   (help "The group-unit is the element of the group for which the unit property holds."))
