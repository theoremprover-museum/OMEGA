...
;; to the Oz representation of OpenMath formulas.
;;
;; also look at   
...
(mod~defmod omcont :uses (post th)
                 :documentation "All KEIM objects have an Oz OpenMath representations, this module has the input/output functions."
		 :exports (
			   omcont~print
			   omcont~string
			   omcont~print-declaration
			   omcont~print-omdoc-declaration
			   omcont~print-omdoc-prefix
			   omcont~print-theory
			   )
		 )
...
(defmethod omcont~print ((obj data+appl) stream)
  (let ((func (data~appl-function obj))
    ...
    (if (and (term~primitive-p func)
             (member (keim~name func) omcont*quantors))
...
(defmethod omcont~print ((obj term+abstr) stream)
  (format stream "'OMBIND'('OMS'(cd: \"mltt\" name: \"lambda\")")
  (format stream "'OMBVAR'(")
...
(defun omcont=defined-in-name (thing)
  (let* ((assoc-cd (assoc (intern (keim~name thing)) omcont*symbol-cd-mapping))
