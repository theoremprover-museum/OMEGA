...
;;     Germany                                                              ;;
;;   electronic mail: omega@cs.uni-sb.de                                    ;;
;;                                                                          ;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; This file contains functions for translating OMEGAs logical entities to 
;; the Mozart internal representation of OMDoc elements.
;; This includes the Oz representation of OpenMath formulas.
;;
;; also look at   
...
(setf omcont*symbol-cd-mapping 
	...
	(GREATER . ("gt" "relation1"))
        ...
        (continuous-at . ("continuous" "calculus2"))

        (NOT    . ("not" "logic1"))
        (FORALL . ("forall" "quant1"))
        (EXISTS . ("exists" "quant1"))
	)
      )

(setf omcont*quantors '(FORALL EXISTS))

(defvar omcont*current-theory-name "arith1")
...
output will be to *standard-output*.")
  ...
  (:method ((object t) stream)
	   (when object (post~print object stream)))
...
(defmethod omcont~print ((obj symbol) stream)
  (format stream "~A" symbol))

(defmethod omcont~print ((obj pdsn+node) stream)
  (let ((id (keim~name obj))
        (hyps (pdsn~hyps obj))
        )
    (format stream "'assertion'(id: \"~A\" type: \"conjecture\"" id)	    
    (format stream " cmp: \"~A\"~&"
	    (omcont=canonicalize-name (format nil "The sequent ~A." id)))
    (format stream " fmp: 'FMP'(assumptions: ")
    (if hyps
        (progn (format stream "[")
               (mapcar #'(lambda (hyp)
                           (format stream "'assumption'(cmps: nil omobj: 'OMOBJ'(")
                           (omcont~print (node~formula hyp) stream)
                           (format stream ")) ")
                           )
                       hyps)
               (format stream "]")
      ...
      (format stream "nil"))
    (format stream " conclusion: 'conclusion'(cmps: nil omobj: 'OMOBJ'(")
...
(defmethod omcont~print ((obj data+variable) stream)
  (format stream "'OMV'(name: \"~A\")" (string-downcase (string (keim~name obj)))))
;;(omcont=canonicalize-name (keim~name obj))))
...
(defmethod omcont~print ((obj data+appl) stream)
  (let ((func (data~appl-function obj))
    ...
    (if (term~primitive-p func)
        (if (member (keim~name func) omcont*quantors)
            (let ((abstr  (first (data~appl-arguments obj)))
                  )
              (format stream "'OMBIND'(")
              (omcont~print func stream)
              (omcont~print (data~abstr-domain abstr) stream)
              (omcont~print (data~abstr-range abstr) stream)
              (format stream ")~&"))
          (if (eql (keim~name func) 'forall-sort)
              (progn ;;(format t "forall-sort: ~A" (first (data~appl-arguments obj)))
                     (format stream "'OMA'('OMS'(cd: \"quant1\" name: \"forall\") [")
                     (omcont~print (first (data~appl-arguments obj)) stream)
                     (format stream "])")
                     )
            (progn 
              (format stream "'OMA'(")
...
(defmethod omcont~print ((obj data+schema) stream)
  (format stream "'OMBIND'('OMS'(cd: \"POST\" name: \"all-types\")")
  (format stream " 'OMBVAR'(")
...
(defmethod omcont~print-declaration ((variable term+variable) stream)
  (format stream "'OMATTR'(['OMATP'('OMS'(cd: \"mltt\" name: \"type\")#~A)] 'OMV'(name: \"~A\"))"
...
(defmethod omcont~print ((obj term+abstr) stream)
  (format stream "'OMBIND'('OMS'(cd: \"fns1\" name: \"lambda\")")
  (format stream " 'OMBVAR'(")
...
(defun omcont=canonicalize-name (symbol-or-string)
  ...
  (let ((string (format nil "~A" symbol-or-string)))
    ...
    (when (eq (elt string 0) #\-)
      (setf string (string-downcase (concatenate 'string "ID" string))))
    (do ((i 0 (1+ i))
	 (resstring ""
		    (let ((char (elt string i)))
		      (cond 
		       ((eq char #\<) (concatenate 'string resstring "LESS"))
		       (T (concatenate 'string resstring (string char)))))))
	((= i (length string)) (string-downcase resstring)))))
...
(defmethod omcont~print ((obj th+axiom) stream)
  (let ((id (omcont=canonicalize-id (keim~name obj))))
    (format stream "'assertion'(id: \"~A\" type: \"axiom\"" id)	    
    (format stream " cmp: \"~A\""
	    (omcont=canonicalize-name (help~help-string obj)))
    (format stream " fmp: 'FMP'(conclusion: 'OMOBJ'(")
    (omcont~print  (th~ass-formula obj) stream)
    (format stream ")))")))
...
(defmethod omcont~print ((obj prob+problem) stream)
  (let* ((name (keim~name obj))
    ...
    (when help (format stream " cmp: \"~A\" " help))
    (format stream " fmp: 'FMP'(assumptions: nil ~&")
    (format stream " conclusion: conclusion(omobj: 'OMOBJ'(")
;;    (format t "conclusion: ~S" (node~formula (prob~conclusion obj)))
    (omcont~print (node~formula (prob~conclusion obj)) stream)
    (format stream "))))")
    ))
...
(defmethod omcont~print ((obj th+theory) stream)
  ...
  (let* ((title      (format nil "The Theory ~A" (keim~name obj)))
    ...
    (setq omcont*current-theory-name id)
...
;;         (adsfasd (omega~message "mapping ~A to ~A" thing assoc-cd))
	...
	(cdr assoc-cd)
      (if (th~defined-in thing)
          (list (keim~name thing) (keim~name (th~defined-in thing)))
        (list (keim~name thing) omcont*current-theory-name)))))
